...
def analyze_resume_with_gemini(resume_text):
    ...
    try:
        response = openai.responses.create(
            model="gemini-1.5",
            input=f"Extract structured resume info from this text:\n{resume_text}"
        )
...
def safe_get(d, key, default="-"):
    """Return value from dict or - if missing"""
    return d.get(key, default)

def calculate_work_experience(experience_list):
    """Sum total experience from work experience list"""
...
def format_skills(experience_list, projects_list):
    """Create skill summary with experience and project count"""
...
def format_work_experience_details(experience_list):
    """Concatenate work experience details"""
    if not experience_list:
        return "-"
    details = []
    for exp in experience_list:
